...
| icone-stencil | `task` | FATTO (v2 Lottie): le 3 card servizi usano i **Lottie Lordicon ORIGINALI** dell'utente (_lottie/*.json → assets/lottie/icon-*.json) RICOLORATI via scipt `_lottie/recolor_lottie.py` (primary→ink, secondary→hole): c-w teal/#161616, c-g lavanda/#FAF7F6, c-l lime/#FAF7F6. Render: **lottie-web** (assets/lottie/lottie.min.js) con play una volta all'ingresso (IntersectionObserver) + replay hover/touch; prefers-reduced-motion → frame finale. ⚠ BUG risolto: vecchio CSS stroke-dashoffset:1600 rendeva le icone INVISIBILI (classe .ico-in su elemento sbagliato). Ordine script: lottie.min.js PRIMA di main-11.js. Rimossi SVG statici scartati (assets/img/icon-*.svg) |
